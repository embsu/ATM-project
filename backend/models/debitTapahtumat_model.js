...
const tapahtumat = {
  getAll: function(callback) {
   return db.query('SELECT idTapahtumat, DATE_FORMAT(Paivamaara,"%a %d.%m.%Y %k:%i") AS Paivamaara, Tyyppi, Maara, idDebit FROM tapahtumat WHERE idDebit IS NOT NULL AND idCredit IS NULL ORDER BY Paivamaara DESC', callback);
  },
  getById: function(kortinNumero, callback) {
      return db.query ('SELECT t.Maara, t.Tyyppi, DATE_FORMAT(t.Paivamaara, "%a %d.%m.%Y %k:%i") AS Paivamaara, t.idDebit FROM tapahtumat t JOIN kortti k ON t.idDebit = k.idDebit WHERE k.Kortin_numero = ? ORDER BY t.Paivamaara DESC LIMIT 5;', [kortinNumero], callback);
  },
